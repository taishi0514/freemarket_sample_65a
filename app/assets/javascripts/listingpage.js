$(document).on('turbolinks:load', function() {
  ...
  $("#parent").on("change",function(){
    ...
    if(str == 0){
      ...
      $("#product_product_size_id").remove();
      $(".items-second__head--third").remove();
    }else{
      $.ajax({
        url: "/api/categories",
        type: 'GET',
  ...
  $(document).on("change","#child",function(){
    ...
    if(strchild == 0){
      ...
      .done(function(categories) {
        var insertHTML = `<select name="product[category_id]" id="item_category_id">
                          <option value=0>---</option>`;
        $.each(categories, function(i, category) {
          insertHTML += buildHTML(category)
        });
  ...
  $(document).on("change","#item_category_id",function(){
    ...
    if(str == 0){
      ...
      .done(function(product_sizes) {
        var insertHTML = `<select name="product[product_size_id]" id="item_product_size_id">`;
  ...
  $("#shippingway").on("change",function(){
    ...
    if(str == 0){
      ...
      .done(function(shippingways) {
          var insertHTML = ` <select name="product[shippingway_id]" id="item_shippingway_id">`;
